...
async function saveRetraining(payload: DirectionWritePayload, editing?: QualificationDirection | null) {
  if (editing?.id) return updateRetrainingDirection(editing.id, payload);
  return createRetrainingDirection(payload);
}

export default function RetrainingDirectionsPage() {
  ...
  const loadDirection = useCallback(async (directionId: number, force = false, silent = false) => {
    ...
    const key = directionKey({ id: directionId, source: "retraining" as const });
    ...
    try {
      const detail = dropRemovedLessonsFromTree(
        await getRetrainingDirection(directionId, false, { fetchMaterials: true })
      );
      setItems((prev) =>
        prev.map((item) => (item.id === directionId ? { ...item, ...detail, source: "retraining" as const } : item))
      );
    } catch (error) {
      setItems((prev) =>
        prev.map((item) => (item.id === directionId ? { ...item, modules: item.modules ?? [] } : item))
      );
  ...
  const loadList = useCallback(async (silent = false) => {
    ...
    try {
      // Faqat ro'yxat — daraxt yoyilganda detail alohida yuklanadi.
      const directions = await getRetrainingDirections({ per_page: 100 });
      const prev = itemsRef.current;
      if (silent && directions.length === 0 && prev.length > 0) return;
      setItems(
        directions.map((item) => {
  ...
  const deleteModule = async (direction: QualificationDirection, qualModule: QualificationModule) => {
    ...
    setSaving(true);
    try {
      await forceDeleteModule(qualModule.id, qualModule.lessons ?? [], undefined, "retraining");
      setItems((prev) =>
        prev.map((item) => {
          if (item.id !== direction.id) return item;
  ...
  const loadLessonMaterials = useCallback(
    async (direction: QualificationDirection, qualModule: QualificationModule, lesson: QualificationLesson) => {
      ...
      try {
        const materials = await getRetrainingLessonMaterials(lesson.id);
        setItems((prev) =>
          prev.map((item) => {
            if (item.id !== direction.id) return item;
            return {
    ...
    <div>
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              href={qualificationWizardPath({ source: "retraining", step: 1 })}
      ...
      {loading ? (
        ...
        <QualificationTree
          ...
          onDeleteLesson={(direction, qualModule, lesson) =>
            setPendingLesson({ direction, module: qualModule, lesson })
          }
          ...
          <div className="space-y-3">
            <label className="block text-sm">
              ...
              <input
                ...
                onChange={(e) =>
                  setModuleForm((prev) => (prev ? { ...prev, editing: { ...prev.editing, title: e.target.value } } : prev))
                }
      ...
      <DirectionFormModal
        ...
        onSaved={async (direction) => {
          ...
          const next = { ...direction, source: "retraining" as const };
          setItems((prev) => {
