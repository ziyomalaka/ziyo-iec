...
export default async function RetrainProcess() {
            ...
            return (
              <div key={label} className="flex items-center gap-1 lg:flex-1">
                <div className="flex flex-col items-center text-center flex-1">
                  <div
                    className={`mb-2 flex h-12 w-12 items-center justify-center rounded-full ${stepColors[index]}`}
                  >
                    <Icon className="h-5 w-5" />
                  </div>
                  <span className="max-w-[90px] text-[11px] font-medium leading-tight text-slate-700">
                    {label}
                  </span>
                </div>
                {index < steps.length - 1 && (
                  <ChevronRight className="hidden h-4 w-4 shrink-0 text-orange-400 lg:block" />
                )}
              </div>
            );
          })}
