...
import { matchPublishedDirection, readQualificationSnapshot } from "@/lib/qualification/published-snapshot";
...
function asLessonFromQual(lesson: QualificationLesson): CourseLessonSummary | null {
  ...
  if (!isVisibleToStudent(lesson.status)) return null;
...
function overlayModule(base: CourseModuleResponse, overlay: CourseModuleResponse, allowNew = true): CourseModuleResponse {
  ...
  for (const lesson of overlay.lessons ?? []) {
    if (!isVisibleToStudent(lesson.status)) continue;
    ...
    else if (allowNew) lessons.push(lesson);
  }
  return {
    ...base,
    title: overlay.title || base.title,
    order_index: overlay.order_index ?? base.order_index,
    lessons: lessons.filter((lesson) => isVisibleToStudent(lesson.status)),
  };
}
...
export async function getCatalogCourse(id: string): Promise<CourseDetailResponse> {
  ...
  const [courseResult, itResult, itCourseResult, learningResult, mergedResult, snapshotResult] = await Promise.allSettled([
    ...
    numeric ? getLearningCourse(numeric) : Promise.reject(new Error("skip")),
  ...
  const snapMatch = numeric ? matchPublishedDirection(snapshot, numeric) : null;
  ...
  const learningModules: CourseModuleResponse[] = (learning?.modules ?? [])
    ...
    .map((module) => ({
      ...
      lessons: (module.lessons ?? module.items ?? [])
        .filter((lesson) => isVisibleToStudent(lesson.status))
        .map((lesson) => ({
          id: lesson.id,
          title: lesson.title,
  ...
  const publishedModules = snapshotModules ?? (adminModules.length ? adminModules : null);
  const modules = publishedModules
    ? enrichAdminModules(publishedModules, [course?.modules, learningModules])
    : mergeCourseModules([course?.modules]);
  ...
  return {
    ...
    title: qualificationDirection?.title || itDirection?.title || course?.title || learning?.title || "",
